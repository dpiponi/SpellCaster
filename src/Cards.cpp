...
void Henge::executeInstant(SpellCaster *game, int c, bool verbose) const {
    game->mana[game->owner[c]] += Mana {4, 1};
    if (verbose) {
        board << "Some mana for you";
        game->end_message();
    }
...
void Church::executeInstant(SpellCaster *game, int c, bool verbose) const {
    int count = 0;
    for (auto p = game->in_play.begin(); p != game->in_play.end(); ++p) {
        count += game->hasProperty(*p, CardProperty::BLESSED);
    }
    game->mana[game->owner[c]] += Mana {1, 4+count};
    if (verbose) {
        board << "Some mana for you";
        game->end_message();
...
void AntiMagicBase::execute(SpellCaster *game, int c, bool verbose) const {
    ...
    game->properties[game->target[c]] |= CardProperty::MAGIC_RESISTANT;
    game->card_end_from(Location::EXECUTING, c, verbose);
}
...
void DeathsDoor::execute(SpellCaster *game, int c, bool verbose) const {
    if (verbose) {
